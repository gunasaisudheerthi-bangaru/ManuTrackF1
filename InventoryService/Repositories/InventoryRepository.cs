...
{
    public async Task<IEnumerable<InventoryItem>> GetAllAsync(string? status = null, string? locationId = null)
    {
        var query = db.InventoryItems.AsQueryable();
        if (!string.IsNullOrWhiteSpace(status)) query = query.Where(i => i.Status == status);
        if (!string.IsNullOrWhiteSpace(locationId)) query = query.Where(i => i.LocationID == locationId);
        return await query.OrderBy(i => i.ProductName).ToListAsync();
    }

    public async Task<InventoryItem?> GetByIdAsync(int id) => await db.InventoryItems.FindAsync(id);

    public async Task<InventoryItem?> GetByProductIdAsync(int productId) =>
        await db.InventoryItems.FirstOrDefaultAsync(i => i.ProductID == productId);

    public async Task<IEnumerable<InventoryItem>> GetLowStockAsync() =>
        await db.InventoryItems.Where(i => i.QuantityOnHand <= i.MinimumQuantity).ToListAsync();
    ...
    public async Task<bool> ExistsAsync(int id) => await db.InventoryItems.AnyAsync(i => i.InventoryID == id);
}
