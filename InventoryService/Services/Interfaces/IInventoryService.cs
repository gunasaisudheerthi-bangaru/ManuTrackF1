...
{
    Task<ApiResponse<IEnumerable<InventoryItemViewModel>>> GetAllAsync(string? status, int? locationId);
    ...
    Task<ApiResponse<IEnumerable<StockMovementViewModel>>> GetMovementsAsync(int inventoryId);
}
